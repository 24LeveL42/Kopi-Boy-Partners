...
async function choosePartner(role){
  ...
  // A returning, already-signed-in partner should never see the sign-in
  // screen again — route them straight in if we can match their account.
  if(kbAuthReady()){
    const user=await kbGetUser();
    if(user){
      const routed=await routeSignedInPartner(user,role);
      if(routed)return;
    }
  }

  // First-time (or not-yet-matched) visitor: sign in with Google once.
  ...
  kbOpenAuth(
    role==="cook"
      ? "Sign in with Google to register as a Cook"
      : "Sign in with Google to register as a Rider"
  );
}

// Look up whether this Google account already belongs to an approved
// partner, a pending application, or neither — and route accordingly.
// Returns true if it fully handled navigation, false if the caller should
// fall back to showing the sign-in screen.
async function routeSignedInPartner(user,role){
  if(role==="cook"){
    ...
    const {data:app}=await supabase.from("cook_applications")
      .select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();
    if(app){showCookApplicationStatus(app);return true;}
    ...
    const {data:app}=await supabase.from("rider_applications")
      .select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();
    if(app){showRiderApplicationStatus(app);return true;}
...
async function autoRouteReturningPartner(){
  ...
  const {data:cookApp}=await supabase.from("cook_applications")
    .select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();
  if(cookApp){showCookApplicationStatus(cookApp);return true;}

  const {data:riderApp}=await supabase.from("rider_applications")
    .select("*").eq("user_id",user.id).order("created_at",{ascending:false}).limit(1).maybeSingle();
  if(riderApp){showRiderApplicationStatus(riderApp);return true;}

  return false; // signed in, but never applied yet — show home to pick a role
}

function showCookApplicationStatus(app){
  go("cookAccess");
  $("cookSelector")?.classList.add("hidden");
  document.querySelector("#cookAccess .green")?.classList.add("hidden");
  if(app.status==="rejected"){
    ...
    document.querySelector("#cookAccess .text-orange")?.classList.remove("hidden");
  }else{
    $("cookAccessStatus").textContent="Application submitted. Waiting for management approval — you'll be notified once it's reviewed.";
    document.querySelector("#cookAccess .text-orange")?.classList.add("hidden");
  }
}

function showRiderApplicationStatus(app){
  go("riderAccess");
  $("riderSelector")?.classList.add("hidden");
  document.querySelector("#riderAccess .orange")?.classList.add("hidden");
  if(app.status==="rejected"){
    ...
    document.querySelector("#riderAccess .text-orange")?.classList.remove("hidden");
  }else{
    ...
    document.querySelector("#riderAccess .text-orange")?.classList.add("hidden");
  }
}
...
async function submitCookApplication(){
  ...
  const user=await kbGetUser();
  if(!user)return toast("Please sign in with Google first");
