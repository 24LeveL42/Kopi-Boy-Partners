...
function go(id){document.querySelectorAll(".screen").forEach(s=>s.classList.remove("active"));if($(id))$(id).classList.add("active");window.scrollTo({top:0,behavior:"smooth"});}
function toast(m){const t=$("toast");if(!t)return;t.textContent=m;t.style.display="block";clearTimeout(window.tt);window.tt=setTimeout(()=>t.style.display="none",1800);}
...
async function session(){
  if(!window.KOPI_SUPABASE_READY){toast("Kopi Boy database not connected");return null;}
...
async function choosePartner(role){
  ...
  if(!kbAuthReady()){toast("Kopi Boy database not connected");return;}
  kbOpenAuth(role==="cook"?"Sign in with Google to register as a Cook":"Sign in with Google to register as a Rider");
}

async function loadCookSelectors(){
  const s=await session();if(!s)return;
  const {data,error}=await supabase.from("merchants").select("id,name,status,active").eq("status","approved").eq("active",true).order("name");
  $("cookSelector").innerHTML="<option value=''>Select your name</option>"+(data||[]).map(c=>`<option value="${c.id}">${esc(c.name)}</option>`).join("");
  if(error)toast("Could not load approved cooks");
}

async function enterCook(){
  const id=$("cookSelector").value;if(!id)return toast("Select your name");
  const {data,error}=await supabase.from("merchants").select("*").eq("id",id).single();
  if(error)return toast("Cook profile not found");
  currentCook=data;$("cookName").textContent=data.name+" · Cook";renderCookProfile();go("cookDashboard");initCook();loadMenu();
}
...
async function submitCookApplication(){
  ...
  const p={
    full_name:$("cookFullName").value.trim(),display_name:$("cookDisplayName").value.trim(),phone:$("cookPhone").value.trim(),
    food_type:$("cookType").value.trim(),bio:$("cookBio").value.trim(),operating_start:$("cookOpen").value,operating_end:$("cookClose").value,
    daily_capacity:Number($("cookCapacity").value||20),service_area:$("cookArea").value.trim(),service_postal_codes:$("cookPostalCodes").value.trim(),
    sfa_licensed:$("cookSfaLicensed").checked,sfa_licence_type:$("cookSfaLicenceType").value.trim(),sfa_licence_number:$("cookSfaLicenceNumber").value.trim(),
    sfa_licence_expiry:$("cookSfaLicenceExpiry").value||null,sfa_document_name:$("cookSfaDocument").files?.[0]?.name||null,
    compliance_ack:true,compliance_ack_version:"2026-08-21",compliance_ack_at:new Date().toISOString(),status:"pending"
  };
  ...
  const {error}=await supabase.from("cook_applications").insert(p);
  if(error)return toast(error.message);
  $("cookAccessStatus").innerHTML="<b>Application submitted ✓</b><br>Management will review your application. We will welcome you here when your Cook Partner account is approved.";
  go("cookAccess");loadCookSelectors();toast("Application submitted ✓");
}
...
function renderCookOrder(o){$("incomingOrderArea").innerHTML=`<div class="incoming-order"><div style="display:flex;justify-content:space-between"><h3>🔔 ${statusLabel(o.status)}</h3><span class="status-pill ${o.status}">${esc(o.status)}</span></div><div>${esc(o.order_number)}</div><div class="incoming-items">${(o.items||[]).map(x=>`${esc(x.name)} <b style="float:right">x${x.qty||1} · $${Number(x.price||0).toFixed(2)}</b>`).join("<br>")}<hr><b>Total</b><b style="float:right">$${Number(o.total||0).toFixed(2)}</b></div>${o.rider_name?`<div class="job-line"><span>Rider</span><b>🛵 ${esc(o.rider_name)}</b></div>`:""}${timeline(o)}${cookAction(o)}</div>`;}
...
async function cookStatus(id,status){const patch={status},now=new Date().toISOString();({accepted:()=>patch.accepted_at=now,declined:()=>patch.declined_at=now,looking_for_rider:()=>patch.rider_requested_at=now,cooking:()=>patch.cooking_at=now,ready:()=>patch.ready_at=now}[status]?.());const {data,error}=await supabase.from("orders").update(patch).eq("id",id).select().single();if(error)return toast(error.message);renderCookOrder(data);toast(statusLabel(status)+" ✓");}

async function loadMenu(){
  if(!currentCook)return;const {data}=await supabase.from("merchants").select("*").eq("id",currentCook.id).single();if(data)currentCook=data;
  $("menuNote").value=currentCook.menu_note||"";$("orderOpen").value=currentCook.order_open||"09:00";$("orderClose").value=currentCook.order_close||"14:00";$("cookStart").value=currentCook.operating_start||"10:00";$("cookEnd").value=currentCook.operating_end||"16:00";updateToggle();loadDishes();
}
...
async function toggleMenu(){const {data,error}=await supabase.from("merchants").update({menu_live:currentCook.menu_live===false}).eq("id",currentCook.id).select().single();if(error)return toast(error.message);currentCook=data;updateToggle();toast(currentCook.menu_live?"Menu live ✓":"Menu turned off");}
...
async function toggleDish(id,active){const {error}=await supabase.from("menu_items").update({active}).eq("id",id);if(error)return toast(error.message);loadDishes();}

async function loadRiderSelectors(){const s=await session();if(!s)return;const {data}=await supabase.from("riders").select("id,name,status,active").eq("status","approved").eq("active",true).order("name");$("riderSelector").innerHTML="<option value=''>Select your name</option>"+(data||[]).map(r=>`<option value="${r.id}">${esc(r.name)}</option>`).join("");}
...
async function submitRiderApplication(){const ack=$("riderComplianceAck").checked;if(!ack)return toast("Please acknowledge the rider requirements");const p={full_name:$("riderFullName").value.trim(),phone:$("riderPhone").value.trim(),vehicle_type:$("riderVehicle").value,operating_area:$("riderArea").value.trim(),compliance_ack:true,compliance_ack_version:"2026-08-21",compliance_ack_at:new Date().toISOString(),eligibility_ack:$("riderEligibilityAck").checked,eligibility_ack_at:$("riderEligibilityAck").checked?new Date().toISOString():null,status:"pending"};if(!p.full_name||!p.phone)return toast("Name and phone required");const {error}=await supabase.from("rider_applications").insert(p);if(error)return toast(error.message);$("riderAccessStatus").innerHTML="<b>Application submitted ✓</b><br>Management will review your application. We will welcome you here when your Rider Partner account is approved.";go("riderAccess");loadRiderSelectors();toast("Application submitted ✓");}
...
async function loadJobs(){const {data,error}=await supabase.from("orders").select("*").eq("status","looking_for_rider").is("rider_id",null).order("created_at",{ascending:false});if(error)return;$("jobCount").textContent=data?.length||0;$("riderJobsArea").innerHTML=data?.length?data.map(o=>`<div class="rider-order-card"><h3>🛵 Delivery Request · ${esc(o.order_number)}</h3><small>Delivery fee $${Number(o.delivery_fee||0).toFixed(2)}</small><div class="job-line"><span>Pick up</span><b>Cook</b></div><div class="job-line"><span>Deliver to</span><b>Customer</b></div><button class="green full" onclick="acceptJob('${o.id}')">ACCEPT DELIVERY</button></div>`).join(""):"<div class='rider-order-card'><small>No delivery jobs available right now.</small></div>";}
async function acceptJob(id){const {data,error}=await supabase.from("orders").update({status:"rider_accepted",rider_id:currentRider.id,rider_name:currentRider.name,rider_accepted_at:new Date().toISOString()}).eq("id",id).eq("status","looking_for_rider").is("rider_id",null).select().single();if(error)return toast("Job already taken");renderRiderJob(data);toast("Delivery accepted ✓");}
function renderRiderJob(o){$("riderJobTitle").textContent=o.order_number;$("riderJobContent").innerHTML=`<div class="rider-order-card"><span class="status-pill ${o.status}">${statusLabel(o.status).toUpperCase()}</span><h3>Delivery for ${esc(currentRider.name)}</h3><div class="job-line"><span>Delivery fee</span><b>$${Number(o.delivery_fee||0).toFixed(2)}</b></div>${timeline(o)}${o.status==="rider_accepted"?`<button class="green full" onclick="riderStatus('${o.id}','cooking')">CONFIRM — WAITING FOR FOOD</button>`:""}${o.status==="ready"?`<button class="green full" onclick="riderStatus('${o.id}','out_for_delivery')">I'VE COLLECTED THE FOOD</button>`:""}${o.status==="out_for_delivery"?`<button class="green full" onclick="riderStatus('${o.id}','delivered')">DELIVERED TO CUSTOMER</button>`:""}</div>`;go("riderJob");}
...
window.KB_AUTH_CONFIG={appRole:document.body?.dataset?.app||"partner",socialProviders:["google","facebook"],phoneOtpReady:false};
...
async function kbSignIn(provider){if(!kbAuthReady())return toast("Kopi Boy database is not connected");sessionStorage.setItem("kb_partner_auth_started","1");const redirectTo=window.location.origin+window.location.pathname;const {error}=await supabase.auth.signInWithOAuth({provider,options:{redirectTo}});if(error){sessionStorage.removeItem("kb_partner_auth_started");toast(error.message);}}
...
async function finishPartnerOAuth(){const pendingRole=localStorage.getItem("kb_partner_pending_role");const authStarted=sessionStorage.getItem("kb_partner_auth_started")==="1";if(!pendingRole||!authStarted||!kbAuthReady())return false;const user=await kbGetUser();if(!user)return false;currentRole=pendingRole;localStorage.removeItem("kb_partner_pending_role");sessionStorage.removeItem("kb_partner_auth_started");kbCloseAuth();if(window.location.search||window.location.hash)history.replaceState({},document.title,window.location.pathname);if(currentRole==="cook")go("cookApply");else go("riderApply");toast("Google account verified ✓");return true;}
function setupPartnerAuthListener(){if(!kbAuthReady())return;supabase.auth.onAuthStateChange((event,session)=>{if(event==="SIGNED_IN"&&session)setTimeout(()=>finishPartnerOAuth(),0);});}
document.addEventListener("DOMContentLoaded",async()=>{setupPartnerAuthListener();setTimeout(()=>finishPartnerOAuth(),150);});
