...
async function choosePartner(role){
  ...
  // Always require the Partner to pass through the authentication screen first.
  // Do not bypass this just because a Google/Supabase session already exists.
  // This guarantees the user sees the Google login option before registration.
...
async function enterCook(){
  const id=$("cookSelector").value;
  if(!id)return toast("Select your name");
...
async function loadRiderSelectors(){
  await session();
  const {data}=await supabase.from("riders").select("id,name,status,active").eq("status","approved").eq("active",true).order("name");
  $("riderSelector").innerHTML="<option value=''>Select your name</option>"+(data||[]).map(r=>`<option value="${r.id}">${esc(r.name)}</option>`).join("");
}

async function enterRider(){
  const id=$("riderSelector").value;
  if(!id)return toast("Select your name");
  const {data,error}=await supabase.from("riders").select("*").eq("id",id).single();
  if(error)return toast("Rider profile not found");
  currentRider=data;
...
async function submitRiderApplication(){
  ...
  const {error}=await supabase.from("rider_applications").insert(p);
  if(error)return toast(error.message);
  $("riderAccessStatus").textContent="Application submitted. Waiting for management approval.";
  go("riderAccess");
  loadRiderSelectors();
  toast("Application submitted ✓");
}
...
document.addEventListener("DOMContentLoaded",async()=>{
  ...
  // Only continue automatically when returning from the OAuth redirect.
  // A plain page refresh must not silently bypass the authentication screen.
  const oauthReturn=window.location.hash.includes("access_token") ||
                    new URLSearchParams(window.location.search).has("code");

  if(!user||!oauthReturn) return;
